...
{% if is_incremental() and 'pangolin' not in var('HEAL_MODELS') %}
WHERE
  _inserted_timestamp >= (
    SELECT
      MAX(_inserted_timestamp) - INTERVAL '{{ var("LOOKBACK", "4 hours") }}'
...
{% if is_incremental() and 'platypus' not in var('HEAL_MODELS') %}
WHERE
  _inserted_timestamp >= (
    SELECT
      MAX(_inserted_timestamp) - INTERVAL '{{ var("LOOKBACK", "4 hours") }}'
...
pharaoh_v2 AS (
  SELECT
    ...
    _inserted_timestamp
  FROM
    {{ ref('silver_dex__pharaoh_v2_swaps') }}
...
ORDER BY
  token_in_is_verified desc, token_out_is_verified desc, _inserted_timestamp DESC)) = 1
