...
WITH nft_base_models AS (
    ...
    SELECT
        ...
        _inserted_timestamp
    FROM
        {{ ref('silver__joepegs_sales') }}

{% if is_incremental() and 'joepegs' not in var('HEAL_MODELS') %}
WHERE
    _inserted_timestamp >= (
        SELECT
...
SELECT
    ...
    _inserted_timestamp
FROM
    {{ ref('silver__salvor_sales') }}

{% if is_incremental() and 'salvor' not in var('HEAL_MODELS') %}
WHERE
    _inserted_timestamp >= (
        SELECT
            MAX(_inserted_timestamp) - INTERVAL '{{ var("LOOKBACK", "4 hours") }}'
...
all_prices AS (
    ...
    SELECT
        HOUR,
        'AVAX' AS symbol,
        'AVAX' AS token_address,
        decimals,
        hourly_prices
    FROM
        prices_raw
    WHERE
        token_address = '0xb31f66aa3c1e785363f0875a1b74e27b85fd66c7' -- wrapped avax
),
avax_price AS (
    SELECT
        HOUR,
        hourly_prices AS avax_price_hourly
    FROM
        prices_raw
    WHERE
...
final_base AS (
    SELECT
        ...
        tx_fee,
        tx_fee * avax_price_hourly AS tx_fee_usd,
        origin_from_address,
        origin_to_address,
        origin_function_signature,
        nft_log_id,
